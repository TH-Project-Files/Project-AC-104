...
def main() -> None:
    ...
    # keep the docs-site landing page statistics in sync with the controls
    igs = Counter(d["ig"] for d in mds.values())
    stats = ("<!-- stats:begin (auto-generated by scripts/regen_csv.py — do not edit by hand) -->\n"
             f"- **Total Controls:** {len(mds)}\n"
             f"- **IG 1 (The Starting Line):** {igs['IG 1']} · "
             f"**IG 2 (Operational AI):** {igs['IG 2']} · "
             f"**IG 3 (Specialized Defense):** {igs['IG 3']}\n"
             "<!-- stats:end -->")
    idx = INDEX_PATH.read_text(encoding="utf-8-sig")
    idx = re.sub(r"<!-- stats:begin.*?stats:end -->", stats, idx, flags=re.S)
    INDEX_PATH.write_text(idx, encoding="utf-8-sig", newline="\n")
