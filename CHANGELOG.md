...
### Added
- `data/AC-104-v1.3.0.csv` — new authoritative index (119 controls) carrying the changes below. `AC-104-v1.2.1.csv` is frozen at its release-day content; the audit prompt and the docs-site download instruction select the highest-versioned file automatically.
- **AI-APP-15** — *Continuously scan public internet sources for exposed organizational secrets, AI API keys, and non-human identity credentials, and automatically revoke on discovery.* (Applications & Data, IG 2, 1-High). Covers the external surfaces that in-pipeline scanning cannot reach — developer and contractor repositories, gists and paste sites, package and container registries, public buckets, mobile bundles, and model-hosting repositories — and mandates automated revocation on discovery rather than deletion of the exposure. Distinct from AI-AUTH-05 (human passwords) and AI-APP-11 (inward-facing secrets baseline); cross-referenced from both. Control count 118 → 119 (IG 2: 62 → 63).
...
- **Smarter Agents Contract v1** — a condensed, copy-paste system-prompt block at the top of the guide (Quick Deploy section), distilled from the Governing Principle, Confidence Model, Claim Labels, Inference Permission Rules, and Output Contract, for direct use in agent instructions, `CLAUDE.md`, or `.cursorrules`.
