...
**Layered with:** AI-NET-08 (enforces gateway-only egress at the network layer — the gateway is only a chokepoint if it cannot be bypassed), AI-APP-11 (secrets baseline preventing provider-key sprawl that bypasses the gateway), AI-GOV-03 (the approved AI service catalog this gateway enforces)  
